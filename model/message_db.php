...
function get_inmessages($id) {
    global $db;
    $query = 'SELECT * FROM Recievers INNER JOIN Texts INNER JOIN Employees ON Recievers.Recv_EM_ID=:id AND Recievers.Text_ID=Texts.Text_ID AND Texts.Msg_SID=Employees.EM_ID ';
    $statement = $db->prepare($query);
     $statement->bindValue(':id',$id);
    $statement->execute();
    $messages = $statement->fetchAll();
